...
const addSource = async () => {
  ...
  try {
    ...
    if (newSource.type === 'mongodb') {
      connection = { uri: newSource.uri, database: newSource.database }
    } else if (newSource.type === 'postgresql') {
      connection = { connectionString: newSource.uri }
    } else if (newSource.type === 'mysql') {
      connection = { uri: newSource.uri }
    } else {
      connection = { filepath: newSource.filepath }
    }
...
<template>
  <div class="space-y-6">
    <form class="grid gap-4 md:grid-cols-2" @submit.prevent="addSource">
      ...
      <label
        v-if="newSource.type === 'sqlite' || newSource.type === 'duckdb'"
      ...
      <label
        v-if="newSource.type === 'postgresql'"
        class="block text-sm font-medium text-gray-700 md:col-span-2"
      >
    ...
    <div class="space-y-4">
          ...
          <div class="flex flex-wrap items-center justify-between gap-4">
            <div class="flex items-start gap-3">
              ...
              <div>
                ...
                <p v-else-if="source.type === 'postgresql' || source.type === 'postgres'" class="text-xs text-gray-500">
      ...
      <p v-if="testMessage" class="text-sm text-gray-600">
        {{ testMessage }}
      </p>
    </div>
